 ...
 */
	solver_params dirichlet1 = {.name = "Dirichlet1", n, .phi = &phi, .g = &g1, save_data, use_multigrid, reltol, .nm_flags = NM_NONE};

/**
 * @brief Solve the poisson equation with only Dirichlet boundary conditions and g(x,y) = g2
 */
	solver_params dirichlet2 = {.name = "Dirichlet2", n, .phi = &phi, .g = &g2, save_data, use_multigrid, reltol, .nm_flags = NM_NONE};

/**
 * @brief Solve the poisson equation with only Neumann boundary conditions and g(x,y) = g1,
 * given phi(0,0) = 0
 */
	solver_params neumann1 = {.name = "Neumann1", n, .phi = NULL, .g = &g1, save_data, use_multigrid, reltol, .nm_flags = NM_ONLY, .value_at_xyz = {0,0,0}};
/**
 * @brief Solve the poisson equation with only Neumann boundary conditions and g(x,y) = g2,
 * given phi(0,0) = 0
 */
	solver_params neumann2 = {.name = "Neumann2", n, .phi = NULL, .g = &g2, save_data, use_multigrid, reltol, .nm_flags = NM_ONLY, .value_at_xyz = {0,0,0}};
